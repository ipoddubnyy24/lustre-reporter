...
from . import daily_report, emf, emf_publish, publish
...
def api_config(cfg: Config, qs: dict) -> dict:
    return {
        ...
        "emf": {
            ...
            "confluence_enabled": bool(((cfg.emf or {}).get("confluence") or {}).get("enabled")),
        },
...
def api_emf_landed(cfg: Config, qs: dict) -> dict:
    ...
    return emf.collect_landed(cfg, tag=tag)


def api_emf_coming(cfg: Config, qs: dict) -> dict:
    """Forecast what lands in each upcoming release (EX Jira items × risk bands)."""
    return emf.collect_coming(cfg)


def api_emf_publish(cfg: Config, qs: dict) -> dict:
    """Build + push the EMF landed + per-line coming pages to Confluence now."""
    return emf_publish.publish_all(cfg)
...
_ROUTES = {
    ...
    "/api/emf/publish": api_emf_publish,
}
